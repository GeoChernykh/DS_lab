...
def scrape_isw_by_date(date):
    ...
    url = f"{base_url}/research/?_date_from={date}%2C{date}&_teams=russia-ukraine"
    ...
    for link in tqdm(page_links, desc="Pages scraped:"):
        try:
            page_resp = requests.get(link, headers=headers)
            page_soup = BeautifulSoup(page_resp.text, 'html.parser')
...
if __name__ == "__main__":
    scrape_isw_by_date(date="2026-03-01")